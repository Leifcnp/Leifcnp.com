...
The wider-view WEB-017 correction is published and publicly verified at [http://leifcnp.com/](http://leifcnp.com/). See [correction evidence](artifacts/phase14-pattern/VERIFICATION.md). The initial Phase 14 bands and first correction were rejected in user review; the wider-view correction is now available for review. Earlier release evidence remains under `artifacts/`. Use the explicit HTTP link for now: HTTPS has a certificate hostname mismatch, tracked separately in WEB-001.
...
Read wind directly from the scene: the Shipyard flag trails downwind. The small coral/cream masthead pennant shows apparent wind, including the boat’s velocity; the HUD continues to describe where wind comes from. Pausing freezes the cues. Reduced motion removes flag flutter while keeping the flags’ direction visible. The original contact spray remains shared with the stronger heel/water response.
...
See [PLAN.md](PLAN.md), the canonical [to-do list](WEBSITE_TODO.md), and the per-task `plans/WEB-NNN/plan.md` files. WEB-017’s wider-view correction is complete. Stop for wave review. WEB-022 shoreline interaction remains queued; WEB-016 harbours/zoom remains deferred.


## Verification and VM graphics

The current site passes 160 native cases and the strict production build. Checks cover independent-layer bounds/derivatives, multi-angle spatial recurrence, hashed triangle parity, actual hull contact and existing sailing/lifecycle behavior. Current evidence is [artifacts/phase14-pattern/VERIFICATION.md](artifacts/phase14-pattern/VERIFICATION.md); exact-SHA Pages, matching public assets and public desktop/mobile controls pass. Earlier Phase 14 evidence remains historical.
