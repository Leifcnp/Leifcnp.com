...
The portfolio is being rebuilt as a small Vite + TypeScript + Three.js scene. WEB-013 / Phase 7 makes the water visibly move with shorter travelling swells, animated crest accents, stronger hull response, and a continuous curved wake with bow foam. The custom boat, shoreline landmarks, accessible scanner, and content drawers remain in place. Content lives in `src/content/portfolio.ts`; `src/content/islands.ts` defines island coordinates, land and docking radii, categories, and content references independently of the renderer.
...
The repository publishes the tracked `docs/` directory from the `master` branch. The release path is to run `pnpm run build`, inspect the generated diff and `docs/CNAME`/`docs/.nojekyll`, smoke-test with `pnpm run preview`, and then commit and push the reviewed output. No Hugo build, theme checkout, configuration, or deploy script is involved. On 2026-09-24 the user reprioritized WEB-013 waves/wake ahead of the storm; this continues the external-testing workflow. Future phases still require review.
