...
The portfolio is being rebuilt as a small Vite + TypeScript + Three.js scene. WEB-011 / Phase 8 adds an offshore storm boundary: navy/slate water, larger shared swells, and a smooth inward current. The reviewed central water and curved wake remain in place. The custom boat, shoreline landmarks, accessible scanner, and content drawers remain in place. Content lives in `src/content/portfolio.ts`; `src/content/islands.ts` defines island coordinates, land and docking radii, categories, and content references independently of the renderer.
...
Sail away from the islands to reach the storm. From the initial heading, holding Forward for roughly 12–15 seconds reaches its transition. Water darkens gradually between radii 142 and 172; the same wave shape grows by up to 40%. The current slows outward travel and carries a released boat back toward calm water. Holding outward thrust can balance the current inside the soft band; steering inward or selecting any scanner category brings you home. The storm adds no automatic steering or teleportation. Pause freezes it; reduced-motion sailing retains the boundary current while optional wave forces/foam remain suppressed. The ordinary opening view and island routes stay calm in the six verified layouts.

See [PLAN.md](PLAN.md), the canonical [to-do list](WEBSITE_TODO.md), and the per-task `plans/WEB-NNN/plan.md` files. WEB-011 adds the offshore storm; stop for review after this release. Wind/sail controls, tighter turns, translucent drawers, submenu harbours and low-priority WEB-017 incoming-wave variation/direction remain separate backlog tasks.
