...
The first WEB-017 correction is published at [http://leifcnp.com/](http://leifcnp.com/). See [correction evidence](artifacts/phase14-refinement/VERIFICATION.md). The initial Phase 14 bands and first correction were rejected in user review; the current wider-view correction is undergoing final verification before publication. Earlier release evidence remains under `artifacts/`. Use the explicit HTTP link for now: HTTPS has a certificate hostname mismatch, tracked separately in WEB-001.
...
See [PLAN.md](PLAN.md), the canonical [to-do list](WEBSITE_TODO.md), and the per-task `plans/WEB-NNN/plan.md` files. WEB-017 is reopened for wider-view repetition. Publish and stop for wave review. WEB-022 shoreline interaction remains queued; WEB-016 harbours/zoom remains deferred.


## Verification and VM graphics

The current WEB-017 correction passes 165 native cases and the strict production build. Checks cover independent-layer bounds/derivatives, multi-angle spatial recurrence, hashed triangle parity, actual hull contact and existing sailing/lifecycle behavior. Current evidence is [artifacts/phase14-pattern/VERIFICATION.md](artifacts/phase14-pattern/VERIFICATION.md); public verification is pending. Earlier Phase 14 evidence remains historical.
