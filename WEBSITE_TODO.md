...
wind cues, WEB-017 wave sets, then WEB-022 shoreline response. WEB-020 is now completed Phase 11; WEB-021 is completed Phase 12; WEB-019 is completed Phase 13; WEB-017’s wider-view correction is published and awaits user review.
...
- [ ] WEB-022 — Make waves visibly interact with islands and shorelines.
  ...
  - Acceptance criteria: Use the shared deterministic wave field to create bounded shoreline response such as small breaking crests, swash, foam, or reflected/dissipated motion at island edges. Keep land geometry, wave sampling, hull contact, wake, storm transitions, and scanner routes coherent; never let water cues imply that the boat can enter land or that island collision rules have changed. Preserve pause/reset, hidden-tab, reduced-motion, WebGL fallback, disposal, and mobile rendering budgets.
  - Progress: Backlog only. This is a future visual/physical interaction task, secondary to direct boat control/feedback. Harbours are deferred; listing shoreline work does not authorize implementation.
  - Next step: When selected, prototype one island and one prevailing wave direction at normal desktop/mobile camera scale, measure shoreline clearance and effect cost, then review before applying the behavior to every island.
  - Plan: [plans/WEB-022/plan.md](plans/WEB-022/plan.md).
...
- [ ] WEB-016 — Zoom into manually reached islands and reveal dockable submenu harbours.
  ...
  - Progress: Deferred by the user on 2026-09-25 while boat feel is refined; no longer next after WEB-015. This extends island data, camera/proximity, shoreline geometry, and content navigation; start with one island after a separate phase selection.
  - Next step: Revisit only after the boat-feel work is reviewed and the user restores harbour/zoom work; then design one island/subsection prototype, resolve camera/trigger and safe harbour-route contracts, and review it before applying the pattern to all islands.
...
No active implementation; awaiting wind-cue and water review.
...
- [x] WEB-019 — Make wind direction visible through flags, wind effects and spray.
  ...
  - Removal completed (2026-09-25): Application commit `9a156695` is published on `origin/master`; [Pages run 36223010394](https://github.com/Leifcnp/Leifcnp.com/actions/runs/36223010394) succeeded for the exact SHA. Public files match the tested build, and public desktop/mobile interactions pass.
  - Review gate: Stop for review. Flags/HUD, wave crests, wake/spray and wind physics remain; no new shoreline/harbour work.
...
- [x] WEB-017 — Vary incoming wave sets while keeping a consistent travel direction.
  ...
  - Initial release (2026-09-25): Application commit `cb100d62` is published on `origin/master`; [Pages run 36220109782](https://github.com/Leifcnp/Leifcnp.com/actions/runs/36220109782) succeeded for the exact SHA.
  - Initial verification: 153 native tests and strict build pass. Three-minute crest/direction checks, original hull/contact limits, fixed-step sailing and wave lifecycle pass; six production layouts and public desktop/mobile controls/content/fallbacks pass. Public assets match the build. See [verification](artifacts/phase14/VERIFICATION.md) and [deployment evidence](artifacts/phase14/public-assets.json).
  - Review feedback (2026-09-25): User rejected the regular parallel bands and lack of natural variation. The earlier overlapping water looked/felt more real. They clarified that some linearity is fine: restore the old interference pattern and layer smooth complexity on top rather than expensive fresh randomness every frame.
  - Correction completed (2026-09-25): Application commit `b5b619a5` is published on `origin/master`; [Pages run 36221055155](https://github.com/Leifcnp/Leifcnp.com/actions/runs/36221055155) succeeded for the exact SHA.
  - Correction verification: 162 native tests, strict build, actual hull/contact/gradient and seeded spatial checks pass. Six final production layouts and public desktop/mobile controls/content/touch/fallbacks pass; public assets match. See [correction evidence](artifacts/phase14-refinement/VERIFICATION.md).
  - Second review (2026-09-25): The user still sees an obvious repeating pattern when zoomed out; subtle shared variation was insufficient.
  - Local verification: 165 native tests, strict build, normal/wide/phone visual comparison, spatial recurrence reduction, independent rendered contact, lifecycle and six production layouts pass. See [wider-view evidence](artifacts/phase14-pattern/VERIFICATION.md).
  - Wider-view correction completed (2026-09-25): Application commit `2061e581` is published on `origin/master`; [Pages run 36222447849](https://github.com/Leifcnp/Leifcnp.com/actions/runs/36222447849) succeeded for the exact SHA. Public HTML/JS/CSS/favicon match the tested build and public desktop/mobile interactions pass.
  - Review gate: Stop for user review of wave appearance/feel. WEB-022 stays queued and WEB-016 harbours/zoom stays deferred.
...
- [x] WEB-021 — Exaggerate reach heeling with a dipping leeward rail and bow spray.
  ...
  - Completed: 2026-09-25. Application commit `d7dac1c` is published on `origin/master`; [Pages run 36217578595](https://github.com/Leifcnp/Leifcnp.com/actions/runs/36217578595) succeeded for the exact SHA.
  - Verification: 137 native tests, strict build, 9,968 actual hull-contact frames, rendered-triangle parity, both tacks/storm, 30/60/120 Hz continuity, six production layouts and public desktop/mobile controls/content/fallbacks pass. Public assets match the build. See [verification](artifacts/phase12/VERIFICATION.md) and [public evidence](artifacts/phase12/public-assets.json).
  - Review gate: Stop for heel/contact review. WEB-019 wind cues is the next candidate; WEB-017 wave variation and WEB-022 shoreline response remain queued. WEB-016 harbours/zoom stays deferred.
...
- [x] WEB-011 — Add an offshore storm boundary that pushes the boat back toward the play area.
  ...
  - Review gate: User authorized Phase 9 wind sailing on 2026-09-24. Physical-device feedback remains welcome. Sustained outward thrust may balance the current in the soft band; releasing or steering inward returns toward calm. Geometry budgets are unchanged; software-VM timing and extreme-aspect-ratio limits are documented. Wind sailing, tighter turns, translucent drawers, subharbours and low-priority WEB-017 remain Backlog.
