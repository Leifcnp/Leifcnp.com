...
- Phase 5 was reviewed and the user authorized the next wave-interaction release on 2026-09-24. WEB-010 / Phase 6 is published and publicly verified: bounded wave forces, hull response, and contact effects. The user subsequently selected WEB-013 clearer waves/wake ahead of WEB-011; Phase 7 is published, publicly verified and accepted. The user then authorized the next obvious step; WEB-011 / Phase 8 offshore storm is active. WEB-017 incoming-wave variation/direction is low-priority Backlog. Complete one phase, verify it, and stop for review before implementing the next phase.
...
- WEB-013 was verified and published through the existing `master` / `docs/` GitHub Pages setup with the domain preserved. The user accepted the swells and authorized the next step; verify and publish WEB-011 through this same workflow, then stop for storm review. A routine local build alone is not blanket authorization for unrelated future publication.
